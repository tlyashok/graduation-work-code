"""
Рекомендательный сервис - итерация 2 (asyncpg + Redis-кэш).

Отличия от итерации 1:
- добавлен Redis-кэш по стратегии cache-aside (см. cache.py);
- recommend() и recommend_popular() сначала смотрят в кэш по
  user_id, при попадании возвращают сохранённый результат.
...
from app import cache, similarity, database, recommender
...
async def lifespan(app: FastAPI):
    # При старте: пул PostgreSQL + пул Redis + загрузка модели сходства в оперативную память
