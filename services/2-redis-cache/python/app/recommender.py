...
Итерация 2 добавляет кэш Redis по стратегии cache-aside (§2.5):
recommend() и recommend_popular() сначала смотрят в кэш по
user_id; при промахе вычисляют результат и кладут его обратно
в кэш с TTL. Сам алгоритм item-based CF не меняется.
"""

from __future__ import annotations

from app import cache, database, similarity
from app.config import REC_MIN_RATINGS

CACHE_KEY_RECOMMENDATIONS = "recommendations"
CACHE_KEY_POPULAR = "popular"
...
async def recommend(user_id: int, n: int = 10) -> list[dict]:
    ...
    """
    cached = await cache.get(CACHE_KEY_RECOMMENDATIONS, user_id, n)
    ...
    result = await _item_based_cf(user_ratings, n)
    await cache.set(CACHE_KEY_RECOMMENDATIONS, user_id, n, result)
    return result
...
async def recommend_popular(
    ...
    cached = await cache.get(CACHE_KEY_POPULAR, user_id, n)
    if cached is not None:
        return cached
    ...
    await cache.set(CACHE_KEY_POPULAR, user_id, n, result)
    return result
