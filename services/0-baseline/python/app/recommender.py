...
def recommend(user_id: int, n: int = 10) -> list[dict]:
    ...
    """
    user_ratings = database.fetch_user_ratings(user_id)

    if len(user_ratings) < REC_MIN_RATINGS:
        return recommend_popular(user_id, n, user_ratings)

    return _item_based_cf(user_ratings, n)
...
def recommend_popular(
    ...
    popular = database.fetch_popular_movies(rated_movie_ids, n)
...
def get_similar(movie_id: int, n: int = 6) -> list[dict]:
    ...
    titles = database.fetch_movie_titles(movie_ids)
...
def _item_based_cf(user_ratings: list[tuple[int, float]], n: int) -> list[dict]:
