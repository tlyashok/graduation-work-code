...
func Recommend(sim *model.SimilarityModel, userID, n int) ([]Recommendation, error) {
	ratings, err := database.FetchUserRatings(userID)
	if err != nil {
		return nil, err
	}

	if len(ratings) < MinRatingsForPersonal {
		return RecommendPopular(userID, n, ratings)
	}

	return itemBasedCF(sim, ratings, n)
}

func RecommendPopular(userID, n int, ratings []database.Rating) ([]Recommendation, error) {
	if ratings == nil {
		var err error
		ratings, err = database.FetchUserRatings(userID)
		if err != nil {
	...
	popular, err := database.FetchPopularMovies(excludeIDs, n)
	if err != nil {
		return nil, err
	}
...
func itemBasedCF(sim *model.SimilarityModel, ratings []database.Rating, n int) ([]Recommendation, error) {
